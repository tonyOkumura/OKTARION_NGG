package com.message_micro.model
...
class CityService(private val connection: Connection) {
    companion object {
        private const val CREATE_TABLE_CITIES =
            "CREATE TABLE IF NOT EXISTS CITIES (ID SERIAL PRIMARY KEY, NAME VARCHAR(255), COUNTRY VARCHAR(255), POPULATION INT);"
        private const val SELECT_CITY_BY_ID = "SELECT name, country, population FROM cities WHERE id = ?"
        private const val INSERT_CITY = "INSERT INTO cities (name, country, population) VALUES (?, ?, ?)"
        private const val UPDATE_CITY = "UPDATE cities SET name = ?, country = ?, population = ? WHERE id = ?"
    ...
    suspend fun create(city: City): Int = withContext(Dispatchers.IO) {
        ...
        statement.setInt(3, city.population)
    ...
    suspend fun read(id: Int): City = withContext(Dispatchers.IO) {
        ...
        if (resultSet.next()) {
            ...
            return@withContext City(name, country, population)
        } else {
